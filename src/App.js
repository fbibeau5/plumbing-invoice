    ...
    return (
      <div style={{ minHeight: '100vh', background: C.bg, fontFamily: 'system-ui,-apple-system,sans-serif', color: C.text, paddingBottom: 'calc(72px + env(safe-area-inset-bottom))' }}>
        ...
        <div style={{ background: C.header, padding: '12px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 50, boxShadow: '0 2px 8px rgba(0,0,0,0.2)' }}>
          <img src={process.env.PUBLIC_URL + '/logo.svg'} alt="Révolution Plomberie" style={{ height: 108, filter: 'brightness(0) invert(1)', display: 'block' }} />
        ...
        {tab === 'rapport' && (
          <div style={{ padding: '20px 16px', maxWidth: 700, margin: '0 auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}><button onClick={() => window.print()} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', background: '#1e40af', border: 'none', borderRadius: 8, color: 'white', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>🖨️ Imprimer PDF</button><button onClick={() => { if (window.confirm('Recommencer le rapport de cette semaine?')) { const now = new Date(); const jan1 = new Date(now.getFullYear(), 0, 1); const week = Math.ceil(((now - jan1) / 86400000 + jan1.getDay() + 1) / 7); const wKey = 'report_' + now.getFullYear() + '_' + String(week).padStart(2, '0'); localStorage.removeItem(wKey); fetch('/api/report-data' + '?key=' + wKey, { method: 'DELETE' }).catch(() => {}); setRapportData({}); } }} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', background: '#dc2626', border: 'none', borderRadius: 8, color: 'white', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>🔄 Recommencer</button></div>
            </div>
